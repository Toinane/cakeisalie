...
public class SpicedCakeBlock extends CakeBlock {

    protected static final VoxelShape[] BITES_TO_SHAPE;

    public SpicedCakeBlock(int totalBites) {
        super(totalBites);
    }

    @Override
    protected ActionResult setEffectForCake(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        player.addStatusEffect(new StatusEffectInstance(StatusEffects.SPEED, 400, 4));
    ...
    static {
        BITES_TO_SHAPE = new VoxelShape[] { Block.createCuboidShape(1D, 0D, 1D, 15D, 5D, 15D),
                Block.createCuboidShape(5D, 0D, 1D, 15D, 5D, 15D), Block.createCuboidShape(9D, 0D, 1D, 15D, 5D, 15D),
                Block.createCuboidShape(13D, 0D, 1D, 15D, 5D, 15D), Block.createCuboidShape(0D, 0D, 0D, 0D, 0D, 0D),
